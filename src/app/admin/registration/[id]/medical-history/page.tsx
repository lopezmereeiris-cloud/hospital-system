...
export default function AdminPatientMedicalHistoryRoute() {
  ...
  return (
    <Box sx={{ maxWidth: 1180, mx: "auto", pb: 3 }}>
      <Box sx={{ display: "flex", alignItems: "center", gap: 0.5, mb: 2.2, flexWrap: "wrap" }}>
        ...
        <NavigateNextRoundedIcon sx={{ fontSize: 16, color: "#9CA3AF" }} />
        <Link
          href={`/admin/registration/${encodeURIComponent(patient.patient_id)}`}
          style={{ textDecoration: "none", fontSize: "0.82rem", fontWeight: 500, color: PURPLE }}
        >
          {patient.patient_id}
        </Link>
        <NavigateNextRoundedIcon sx={{ fontSize: 16, color: "#9CA3AF" }} />
        <Chip
          ...
          size="small"
          sx={{ bgcolor: "rgba(67,97,238,0.08)", color: PURPLE, fontWeight: 600 }}
        />
      </Box>
      ...
      <PatientMedicalHistoryPage
        ...
        profileHref={`/admin/registration/${encodeURIComponent(patient.patient_id)}`}
      />
    </Box>
  );
}
